def create_chunks(doc_tuple, md_splitter, py_splitter, txt_splitter, html_splitter):
    doc_chunks = []
    for each in doc_tuple[0]:
        source = each.metadata.get('source', '')
        if source.endswith('.py') and py_splitter:
        ...
        elif source.endswith('.md') and md_splitter:
            ...
            for chunk in md_chunks:
                if isinstance(chunk, str) and chunk.strip():
                    ...
                    doc_chunk.metadata.update(doc_tuple[1])
                    doc_chunks.append(doc_chunk)
        elif (source.endswith('.html') or source.endswith('.htm')) and html_splitter:
            # Use html_splitter for HTML files
            html_chunks = html_splitter.split_text(each.page_content)
            for chunk in html_chunks:
                if isinstance(chunk, str) and chunk.strip():
                    doc_chunk = type(each)(page_content=chunk, metadata=each.metadata)
                    doc_chunk.metadata.update({'type': 'html'})
                    doc_chunk.metadata.update(doc_tuple[1])
                    doc_chunks.append(doc_chunk)
        else:
